...
const NotificationSequence yellow_led_sequence = {
    &message_green_255, &message_red_255,   &message_do_not_reset,
    &message_vibro_on,  &message_note_c5,   &message_delay_50,
    &message_vibro_off, &message_sound_off, NULL,
};

const NotificationSequence red_led_sequence = {
    &message_red_255,   &message_do_not_reset,
    &message_vibro_on,  &message_note_c5,
    &message_delay_50,  &message_vibro_off,
...
MHZ19App *mh_z19app_alloc() {
  ...
  view_port_input_callback_set(app->view_port, mh_z19app_input_callback,
                               app->event_queue);
  ...
  furi_hal_gpio_init(app->input_pin, GpioModeInput, GpioPullUp,
                     GpioSpeedVeryHigh);
...
int32_t calculate_ppm(int32_t *prevVal, int32_t val, int32_t *th, int32_t *tl,
                      int32_t *h, int32_t *l, SensorRange range) {
...
int32_t mh_z19_app(void *p) {
  ...
  for (bool processing = true; processing;) {
    furi_mutex_acquire(app->mutex, FuriWaitForever);
    ppm = calculate_ppm(&prevVal, furi_hal_gpio_read(app->input_pin), &th, &tl,
                        &h, &l, app->sensor_range);
    if (ppm > 0) {
      app->co2_ppm = ppm;
    }
